...
In the example, the query part is summarized as the steps below:
	...
	Concatenate the values that match k2
Ignore the values that don’t match or random
	Check Stash 
	Return [v3, v1, v2]
...
For the project development, the volume-hiding STE scheme for multi-maps vhMM is followed by these steps mentioned in the paper. The construction will use pseudorandom family of functions and an IND-CPA secure encryption scheme. The steps extracted from the paper are:
